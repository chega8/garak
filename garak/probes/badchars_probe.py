...
class JBBBadCharsProbe(Probe):
    ...
    def _load_prompts(self, path: Path, prompt_field: str):
        self._seeds = []

        if path.suffix.lower() == ".jsonl":
            for line in path.read_text(encoding="utf-8").splitlines():
                if line.strip():
                    self._seeds.append(json.loads(line)[prompt_field])
        elif path.suffix.lower() == ".csv":
            import csv

            with open(path, encoding="utf-8") as f:
                for row in csv.DictReader(f):
                    self._seeds.append(row[prompt_field])
        else:
            raise PluginConfigurationError(
                "Unsupported corpus format. Use .csv or .jsonl with the configured prompt field."
            )
